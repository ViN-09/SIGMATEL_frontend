// src/List_ba_to_approved.jsx
import React, { useState, useEffect } from 'react';
import 'bootstrap-icons/font/bootstrap-icons.css';
import { fetchPendingBa, formatDate, getStatusBadge, approveBA } from './auth';
import { getUSER } from '../auth';
import FullBaModal from './FullBaModal';
import './List_berita_acara_barang.css';

const List_ba_to_approved = () => {
    ...
    const loadData = async (page = 1) => {
        setLoading(true);
        setError(null);
        const result = await fetchPendingBa(user.id, user.jabatan);
    ...
    const handleApprove = async (id, user) => {
        try {
            const res = await approveBA(id, user);
            if (res.success) {
                alert('Berhasil approve BA!');
    ...
    const handleReject = async (id, user) => {
        console.log(`Reject BA id=${id} oleh ${user.jabatan}`);
        // Implementasi API reject bisa ditambah di sini
        loadData(pagination.current_page);
    };
        ...
        <div className="ba-container">
            ...
            {pagination.total > 0 && (
                <div className="ba-info-text">
                    ...
                    Menampilkan {pagination.from} - {pagination.to} dari {pagination.total} data pending
                </div>
            )}
...
export default List_ba_to_approved;